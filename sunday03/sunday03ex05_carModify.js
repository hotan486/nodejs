...
function selectCar(database, _id, callback) {
    var car = database.collection('car');
    
    car.findOne({_id:new ObjectID(_id)}, function(err, car){
        console.log(car);
        if(err) {
            console.log('car find error!');
            callback(err, null);
...
router.route('/process/product/:_id/delete').get(function(req, res) {
    console.log('get /process/product/:_id/delete');
    var _id = req.query._id;
    res.end(_id);
});

router.route('/process/product/:_id/modify').get(function(req, res) {
    console.log('get /process/product/:_id/modify');
    var _id = req.query._id;
    res.end(_id);
});

router.route('/process/product/:_id/modify').post(function(req, res) {
    console.log('post /process/product/:_id/modify');
    var _id = req.query._id;
    res.end(_id);    
});
...
router.route('/process/product/:_id').get(function(req, res) {
    ...
    if(db){
        selectCar(db, _id, function(err, car){
            if(err) throw err;
            
            if(car){
                req.app.render('product_detail', {car:car}, function(err, html){
                    if(err) throw err;
